...
# aa_list = ['LYS', 'HIS', 'GLU', 'ARG', 'VAL', 'SER', 'PRO', 'PHE', 'MET', 'LEU', 'ILE', 'GLY', 'CYS', 'ASP', 'ALA', 'TRP', 'GLN', 'ASN', 'TYR', 'THR']
aa_list = ['HIS', 'TRP']
Incomplete_residue = {'LYS': 84, 'HIS': 120, 'GLU': 84, 'ARG': 165, 'VAL': 35, 'SER': 20, 'PRO': 35, 'PHE': 165,
                      ...
                      'ASN': 56, 'TYR': 220, 'THR': 35}
# We can chnage amino acid
df = pd.read_csv('sample_details_receptor_level3.csv')
...
outputFile4 = open(f'positive_negative_key_list.txt', 'w')
outputFile4.writelines('aa\tdistinct_positive\tdistinct_negative\ttotal_positive\ttotal_negative\tdistinct_matched_percentage\tTotal_matched_percentage\n')
for AminoAcidName in aa_list:
    ...
    Output_Folder_Path = f'/ddnA/work/wxx6941/TSR/code/code/Tarikul_code/Drug/{AminoAcidName.lower()}/output_receptor_level3_mirror_image_count_aa'
    #os.mkdir(Output_Folder_Path)

    # Output_Folder_Path = f'/ddnA/work/wxx6941/TSR/code/code/Tarikul_code/Drug/{AminoAcidName.lower()}/sti/output'
    ...
    for i in range(len(PDB_list)):
        ...
        PDB_File_Path = "/ddnA/work/wxx6941/TSR/code/code/Tarikul_code/Drug/all_aa/receptor_level3/original_pdb_hydrogen_removal/{}.pdb".format(
            PDB_ID)
        p = Bio.PDB.PDBParser()
            ...
            if chain.id == Chain_Name:
                for residue in chain:
                    if str(residue)[9:12] == AminoAcidName.upper():
                        ...
                        outputFile1 = open(
                            f'{Output_Folder_Path}/{PDB_ID}_{Chain_Name}_{AminoAcidName}_{Res_Id}_{X1_end}.triplets_theta29_dist17',
                            'w')
                        outputFile2 = open(
                            f'{Output_Folder_Path}/key_{PDB_ID}_{Chain_Name}_{AminoAcidName}_{Res_Id}_{X1_end}.keys_theta29_dist17',
                            'w')
                        # header
                        ...
                        for i in range(len(xCoord)):
                            for j in range(i + 1, len(yCoord)):
                                for k in range(j + 1, len(zCoord)):
                                    ...
                                    outputFile1.write(
                                        "  {:.2f},{:.2f},{:.2f}   {:.2f},{:.2f},{:.2f}  ".format(xCoord[i], yCoord[i],
                                                                                                 zCoord[i], xCoord[j],
                                                                                                 yCoord[j],
                                                                                                 zCoord[j]))
                        ...
                        keyDict3D_list = list(keyDict3D.keys())
                        sum_ = 0
                        Total_sum = 0
                        
                        for i in range(0, len(keyDict3D_list)):
                            for j in range(i + 1, len(keyDict3D_list)):
                                if abs(int(keyDict3D_list[i])) == abs(int(keyDict3D_list[j])):
                                    sum_ += 2
                                    Total_sum += keyDict3D[keyDict3D_list[i]] + keyDict3D[keyDict3D_list[j]]


                        if sum(keyDict3D.values()) == Incomplete_residue[AminoAcidName]:
                            distinct_matched_percentage='{:.2f}'.format((sum_ / len(keyDict3D)) * 100)
                            Total_matched_percentage= '{:.2f}'.format((Total_sum/ sum(keyDict3D.values())) * 100)
                            ...
                            outputFile4.write(
                                f'{PDB_ID}_{Chain_Name}_{AminoAcidName}_{Res_Id}_{X1_end}\t{distinct_positive}\t{distinct_negative}\t{total_positive}\t{total_negative}\t{distinct_matched_percentage}\t{Total_matched_percentage}\n')
