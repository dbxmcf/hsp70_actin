...
\foreach \y [count=\yi] in {0,...,\nrowblocks}
 \foreach \x [count=\xxi] in {\y,...,\nrowblocks}
 {
    ...
    \node [darkstyle]  (\x\y) at ($ (blklabelstart) + (\blksize*\x ,-\blksize*\y) $){\footnotesize$proc_{\lblct}$};
 }
...
row sep=0.05cm,column sep=0.05cm] (m) at (rrectctr) {
  (\epsilon_{m+1},\epsilon_{m+1}) & \cdots & (\epsilon_{2m+1},\epsilon_{m+1}) \\
  & \ddots & \vdots \\
  & & (\epsilon_{2m+1},\epsilon_{2m+1}) \\
      };
