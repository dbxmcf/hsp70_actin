...
\coordinate (psixbb) at ($ (psixba) + (-\blksize,-\blksize/2)$);
\coordinate (psixbc) at ($ (psixba) + (-\blksize/2,-\blksize/2)$);
\coordinate (cornerbl) at ($ (psixbb) + (-\blksize/4,-1.25*\blksize)$);
%\coordinate (cornertr) at ($ (cornerbl) + (1.5*\blksize,\blksize)$);
\coordinate (rrectsize) at ($ (1.5*\blksize,1.25*\blksize)$);
%\draw [black] (psixba) -- (psixbb);
\draw [black,very thick] (psixba) -- (psixbc);
%\draw [black] (cornerbl) -- (cornertr);
%\draw [white] (psixbb) -- (psixbc);

%\draw [rounded corners=2ex] (4,0) ++(-.25,-.25) rectangle ++(3,3) node [midway] {\footnotesize\texttt{}};
\draw [rounded corners=1ex] (cornerbl)  rectangle ++(rrectsize) node [midway] {\footnotesize\texttt{}};
