...
\coordinate (rrectsize) at ($ (1.6*\blksize,1*\blksize)$);
\coordinate (rrectctr) at ($ (cornerbl) + 0.5*(rrectsize)$);

%\draw [black] (psixba) -- (psixbb);
\draw [black,very thick] (psixba) -- (psixbb);
...
row sep=0.05cm,column sep=0.05cm] (m) at (rrectctr) {
  (\epsilon_i,\epsilon_j) & \cdots & (\epsilon_{i+m},\epsilon_j) \\
  & \ddots & \vdots \\
  & & (\epsilon_{i+m},\epsilon_{j+m}) \\
      };
%};
