...
def processFiles(fileName):
        ...
        return (ntpath.basename(fileName)[:-4],protLen,len(filesDict), sum(list(filesDict.values())))
...
if __name__ == '__main__':
    ...
    df2 = pd.DataFrame(result,columns = ['protein','#amino acids','#keys', '#keys_with_freq'])
